...
export function PricingTiers() {
  ...
  return (
    <div ref={containerRef} className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8 max-w-7xl mx-auto mb-20">
      {PRICING_TIERS.map((tier) => (
        ...
        >
          ...
          >
            {tier.name === "UberX" ? "Drive UberX" : `Select ${tier.name}`}
          </Button>
        </div>
      ))}
    </div>
